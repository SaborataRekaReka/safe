...
- Страница `/partner_link` содержит собственные встроенные стили и логику перехода: загрузка общего CSS, аналитики и доступность cookie не должны останавливать редирект. Стили этой страницы обновляются непосредственно в `partner_link.html`.
...
- Регрессионная проверка загрузки и атрибуции: `node --test tests/partner-tracking.test.js tests/availability.test.js`.
